...
Imagine Priya manages **12 clothing stores**. Every Sunday she receives a giant Excel file
telling her: what products are in each store, how much is coming, how fast each product
...
5. **Download the transfer list and order list** (CSV buttons) and hand them to your team.
   Next week, repeat — your confirmed answers carry over.
...
Priya is the planning head of a 12-store Indian apparel chain. Her Sunday file has
...
Her file is deliberately realistic. It contains values like `1,240` and `₹2,499` and
...
Priya's result: **361 − 10 broken = 351 usable rows.**
...
**HEALTHY**. Priya: 10 ÷ 361 ≈ 2.8% → 🟢 healthy.
...
**Headline health.** Priya's file: **action rate 42.7%** (rows needing a move), **excess
...
- **Incoming isn't automatically safe:** if what's coming is still below the danger line →
  *"top-up order needed"*; below half → *"order more now"*; arriving later than a fresh
  order would → *"the shipment is too late"*; no arrival date → the assumption is disclosed.
...
₹57,528 instead of ₹1,43,820. Across Priya's whole file: gross purchase need **₹1.63
...
- **Broken size runs:** a rack with S, L, XL but **no M** looks stocked — but M-customers
  ...
  and flags these (Priya's file: 10 breaks).
...
<img src="assets/screenshots/hero_dark.png" alt="The cockpit in dark mode: five KPI cards, seven view tabs, search, and the ranked action list" width="100%">

Left to right across the five cards:
...
Top-right corner: the light/dark toggle, and the run stamp — file name, how many rows were
processed vs set aside, and the date the data was true.

Below that: **seven tabs** (the same rows, seven different questions), a search box, six
colored counters you can tap to filter by status, and the two **CSV download buttons** —
the transfer list and order list you hand to your team.

### The action list — what each row tells you

Each row in **Today's Actions** (the default tab, worst-first) reads left to right:
the **status pill** (⚫ out of stock…) · the **product** (SKU code + plain name, colour,
size) · the **store** · a small **fuel-gauge bar** (how full the pipeline is vs the
buffer, with a tick at the danger line — `pipe 0 · buf 74 · 0 days` means empty tank) ·
and the **number that matters**: how many units to order.

### Click any row — the full story behind one number

<img src="assets/screenshots/drawer_light.png" alt="The row drawer: reason, recommended action, inputs, computed values, and what-if sliders" width="100%">
...
daily sales, lead time) · the **computed values** (pipeline, buffer, danger line, days of
stock — "recomputed from raw, never trusted from the file") · and the **what-if sliders**:
drag the sales rate or lead time and watch this one row's status recompute. Marked *"this
row only — plan totals unchanged"* — it's a thought experiment, not an edit.

### The Transfers tab — move stock before buying stock

<img src="assets/screenshots/transfers_light.png" alt="The transfers view: who sends what to whom, with value moved and estimated saving per move" width="100%">
...
beats a supplier lead time and costs nothing new.

### The Signal-fixes tab — where your master data is lying

<img src="assets/screenshots/signal_light.png" alt="The signal fixes view: stated vs actual daily sales rate per product-store, with deviation and confidence" width="100%">
...
<img src="assets/screenshots/quarantine_light.png" alt="The quarantine view: each set-aside row with its reason and a suggested answer" width="100%">
...
**The tests.** A 53-test suite runs before every release. It pins Priya's exact reference
numbers (351/10, 42.7%, ₹1.16 cr, 83 transfers…) *and* the business laws that must never
break: a donor never dips below its buffer, a healthy row never gets an order, totals
always reconcile, nothing is ever recommended from a quarantined row.
...
```bash
make test    # 53-test suite: goldens, invariants, reproducibility
